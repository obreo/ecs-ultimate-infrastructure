...
resource "aws_codebuild_project" "codebuild" {
  count         = var.include_codebuild == true ? 1 : 0
  name          = var.name[0]
  description   = "${var.name[0]}-application"
...
*/
  ...
  source {
    type = "NO_SOURCE"
    buildspec = var.enable_service_connect ? (
      var.enable_nginx[0] ? local.backend_service_connect : local.backend_service_connect_non_nginx
      ) : (
      var.enable_nginx[0] ? local.buildspec_ecs_fargate : local.buildspec_ecs_fargate_non_nginx
    )
  }
  ...
  /* source {
    ...
    buildspec = var.fargate_cluster ? (
      ...
      var.enable_service_connect ? (
        ...
        var.enable_nginx[0] ? local.buildspec_ecs_ec2 : local.buildspec_ecs_ec2_non_nginx
      )
    )
  } */
}
